...
def backwarp(tenIn, tenflow):
    ...
    grid = torch.cat([tenHor, tenVer], 1).to(tenIn.device)
    # end
...
class MetricNet(nn.Module):
    ...
    def forward(self, img0, img1, flow01, flow10):
        ...
        metric0 = (img0 - backwarp(img1, flow01)).abs().mean([1], True)
        metric1 = (img1 - backwarp(img0, flow10)).abs().mean([1], True)

        fwd_occ, bwd_occ = forward_backward_consistency_check(flow01, flow10)
        flow01x = flow01[:, 0, :, :] / ((flow01.shape[3] - 1.0) / 2.0)
        flow01y = flow01[:, 1, :, :] / ((flow01.shape[2] - 1.0) / 2.0)
        flow01 = torch.cat([flow01x.unsqueeze(1), flow01y.unsqueeze(1)], 1)

        flow10x = flow10[:, 0, :, :] / ((flow10.shape[3] - 1.0) / 2.0)
        flow10y = flow10[:, 1, :, :] / ((flow10.shape[2] - 1.0) / 2.0)
        flow10 = torch.cat([flow10x.unsqueeze(1), flow10y.unsqueeze(1)], 1)
